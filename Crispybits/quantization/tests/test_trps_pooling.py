...
def test_cvar_is_pooled_over_all_tokens():
    torch.manual_seed(0)
    # two "samples" of very different length and tail behaviour
    d1 = torch.rand(20)                    # short, mild
    d2 = torch.rand(2000) * 5.0            # long, heavy
    ...
    assert abs(r.mean_local - d.mean().item()) < 1e-5          # token-weighted
    assert abs(r.cvar_tail - cvar(d, 0.10).item()) < 1e-5      # pooled CVaR
    assert abs(r.mean_prop - p.mean().item()) < 1e-5
    assert abs(r.trps - (r.mean_local + 0.25*r.cvar_tail + 0.25*r.mean_prop)) < 1e-6
    # and it differs from the old per-sample averaging
